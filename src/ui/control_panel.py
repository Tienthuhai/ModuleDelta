...
class ControlPanel(ctk.CTkFrame):
    ...
    def __init__(self, master, on_start_callback, on_stop_callback, **kwargs):
        ...
        self.lbl_source_title = ctk.CTkLabel(
            self.source_group, text=self._texts["ctrl_source_title"],
            font=ctk.CTkFont(size=12, weight="bold")
        )
        self.lbl_source_title.grid(row=0, column=0, columnspan=2, padx=10, pady=(5, 2), sticky="w")

        self.source_var = ctk.IntVar(value=0)  # 0 = Camera, 1 = Video
        self.rad_camera = ctk.CTkRadioButton(
            self.source_group, text=self._texts["ctrl_webcam"],
            variable=self.source_var, value=0, command=self.toggle_source
        )
        self.rad_camera.grid(row=1, column=0, padx=10, pady=5, sticky="w")

        self.rad_video = ctk.CTkRadioButton(
            self.source_group, text=self._texts["ctrl_video_file"],
            variable=self.source_var, value=1, command=self.toggle_source
        )
        self.rad_video.grid(row=1, column=1, padx=10, pady=5, sticky="w")

        self.btn_browse_video = ctk.CTkButton(
            self.source_group, text=self._texts["ctrl_browse_video"],
            command=self.browse_video, height=22, state="disabled"
        )
        self.btn_browse_video.grid(row=2, column=0, columnspan=2, padx=10, pady=5, sticky="ew")

        self.lbl_video_status = ctk.CTkLabel(
            self.source_group, text=self._texts["ctrl_no_video"],
        ...
        self.lbl_ai_title = ctk.CTkLabel(
            self.ai_group, text=self._texts["ctrl_ai_title"],
            font=ctk.CTkFont(size=12, weight="bold")
        )
        self.lbl_ai_title.pack(padx=10, pady=(5, 2), anchor="w")

        self.btn_load_model = ctk.CTkButton(
            self.ai_group, text=self._texts["ctrl_load_model_btn"],
            fg_color="#4CAF50", hover_color="#45a049", command=self.browse_model
        )
        self.btn_load_model.pack(fill=ctk.X, padx=10, pady=5)

        self.lbl_model_status = ctk.CTkLabel(
            self.ai_group, text=self._texts["ctrl_no_model"],
            text_color="gray", font=ctk.CTkFont(size=11)
        )
        self.lbl_model_status.pack(padx=10, pady=2, anchor="w")

        # Chọn thiết bị CPU/GPU
        self.lbl_device = ctk.CTkLabel(self.ai_group, text=self._texts["ctrl_device_label"])
        ...
        self.lbl_slider_title = ctk.CTkLabel(
            self.slider_group, text=self._texts["ctrl_slider_title"],
            font=ctk.CTkFont(size=12, weight="bold")
        )
        self.lbl_slider_title.pack(padx=10, pady=(5, 2), anchor="w")

        # Slider Confidence
        self.lbl_conf = ctk.CTkLabel(self.slider_group, text=self._texts["ctrl_conf_default"])
        self.lbl_conf.pack(padx=10, pady=(5, 0), anchor="w")

        self.slider_conf = ctk.CTkSlider(
            self.slider_group, from_=0.1, to=1.0,
            number_of_steps=18, command=self.update_conf_label
        )
        self.slider_conf.set(0.55)
        self.slider_conf.pack(fill=ctk.X, padx=10, pady=5)

        # Slider IoU
        self.lbl_iou = ctk.CTkLabel(self.slider_group, text=self._texts["ctrl_iou_default"])
        self.lbl_iou.pack(padx=10, pady=(5, 0), anchor="w")

        self.slider_iou = ctk.CTkSlider(
            self.slider_group, from_=0.1, to=1.0,
            number_of_steps=18, command=self.update_iou_label
        )
        ...
        self.btn_start = ctk.CTkButton(
            self.action_group, text=self._texts["ctrl_btn_start"],
            fg_color="#2196F3", hover_color="#1976D2", command=self.on_start_callback
        )
        self.btn_start.pack(side=ctk.LEFT, expand=True, fill=ctk.X, padx=(0, 5))

        self.btn_stop = ctk.CTkButton(
            self.action_group, text=self._texts["ctrl_btn_stop"],
            fg_color="#f44336", hover_color="#d32f2f", command=self.on_stop_callback
        )
    ...
    def apply_lang(self, texts: dict):
        ...
        self._texts = texts
        self.title_label.configure(text=texts["ctrl_title"])
        self.lbl_source_title.configure(text=texts["ctrl_source_title"])
        self.rad_camera.configure(text=texts["ctrl_webcam"])
        self.rad_video.configure(text=texts["ctrl_video_file"])
        self.btn_browse_video.configure(text=texts["ctrl_browse_video"])
        self.lbl_ai_title.configure(text=texts["ctrl_ai_title"])
        self.btn_load_model.configure(text=texts["ctrl_load_model_btn"])
        self.lbl_device.configure(text=texts["ctrl_device_label"])
        self.lbl_slider_title.configure(text=texts["ctrl_slider_title"])
        self.btn_start.configure(text=texts["ctrl_btn_start"])
        self.btn_stop.configure(text=texts["ctrl_btn_stop"])
        # Cập nhật giá trị slider hiện tại
        self.update_conf_label(self.slider_conf.get())
        self.update_iou_label(self.slider_iou.get())
        # Chỉ reset status nếu chưa có file
        if not self.video_filepath:
            self.lbl_video_status.configure(text=texts["ctrl_no_video"])
        if not self.model_path:
            self.lbl_model_status.configure(text=texts["ctrl_no_model"])
    ...
    def browse_video(self):
        ...
        file_path = filedialog.askopenfilename(
            title=self._texts["ctrl_dlg_video"],
        ...
        if file_path:
            ...
            self.lbl_video_status.configure(
                text=self._texts["ctrl_video_loaded"].format(filename=filename),
                text_color="#2196F3"
            )
    ...
    def browse_model(self):
        ...
        file_path = filedialog.askopenfilename(
            title=self._texts["ctrl_dlg_model"],
        ...
        if file_path:
            ...
            self.lbl_model_status.configure(
                text=self._texts["ctrl_model_loaded"].format(filename=filename),
                text_color="#4CAF50"
            )

    def update_conf_label(self, val):
        """Cập nhật nhãn hiển thị giá trị Conf."""
        self.lbl_conf.configure(text=self._texts["ctrl_conf_fmt"].format(val=val))

    def update_iou_label(self, val):
        """Cập nhật nhãn hiển thị giá trị IoU."""
        self.lbl_iou.configure(text=self._texts["ctrl_iou_fmt"].format(val=val))
